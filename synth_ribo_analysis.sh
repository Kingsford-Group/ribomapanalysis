...
ribo_cmd="${ribomap_dir}scripts/run_ribomap.sh --rnaseq_fq ${rnaseq_fq} --riboseq_fq ${riboseq_fq_prefix}$e.fq.gz --transcript_fa ${transcript_fa} --cds_range ${cds_range} --softClipping false --adapter ${adapter} --min_fplen ${min_fplen} --max_fplen ${max_fplen} --offset ${offset} --work_dir ${work_dir} --star_idx_dir ${star_idx_dir} --sailfish_dir ${sm_odir}"
